...
//BEGIN INPUT FOR DESCRIPTION OF DINOS
const tRex = {
  oName: "tyrannosaurus",
  diet: "carnivorous",
  weight: "7000kg",
...
console.log(`There are a ${uni.length} colleges that contain the: UNI`)
console.log(uni);
...
zooAnimals.forEach(function(z) {
  animalNames.push({Name: z.animal_name, Scientific: z.scientific_name});
})
...
zooAnimals.filter(function(y){
  if (y.population < 5 ){
    largerPopulation.push(y.animal_name)
  }
})
...
let tPopulation = zooAnimals.reduce(function(populationTotal, current){
  return populationTotal + current.population;
})

console.log(`Total is: ${tPopulation}`);
